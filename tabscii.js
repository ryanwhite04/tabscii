...
string.prototype.getTextInFront = function(){
  ...
  editor.selection.setSelectionRange({
    start:{row:this.position.row,column:this.position.column + 8},
    end:{row:this.position.row,column:this.position.column}
  },false);
...
string.prototype.getTextToCursor = function(){
  this.position = this.getPosition();
  editor.selection.setSelectionRange({
    start:{row:this.position.row,column:0},
    end:{row:this.position.row,column:this.position.column}
  },false);
  var text = editor.getCopyText();
  editor.clearSelection();
...
Format.prototype.changeBase = function(){
  for ( var i=10; i<36; i++ ){
	editor.findAll(i + '([0-9A-Za-z\\-])',{regExp:true});
...
Format.prototype.swap = function(){
    for ( var i=0, n = this.swapIn.length; i < n; i++ ){
      editor.findAll(this.swapOut[i],{ regExp:true});
...
Format.prototype.timing = function(){
  ...
  for (var i = 0, l = this.letters.length; i < l; i++ ){
    ...
    var amount = editor.findAll( this.letters[i] + '(\\s{' + this.numbers[i] + ',}||\\s{0,' + less + '})(?=[WHQES\\|])',{regExp:true, caseSensitive:true});
    editor.selection.toSingleRange();
    editor.clearSelection();
    for (var j = 0; j < amount; j++){
      editor.find( this.letters[i] + '(\\s{' + this.numbers[i] + ',}||\\s{0,' + less + '})(?=[WHQES\\|])',{regExp:true, caseSensitive:true});
...
Format.prototype.timingGP = function(){
  editor.find('[SEQHW]\\.?\\s',{regExp:true});
  editor.clearSelection();
...
Format.prototype.setTiming = function(){
  var place = editor.find(/\s+S*[SEQHW]\.?\s/,{regExp:true, wrap:false});
  if (place !== undefined){
  editor.clearSelection();
...
$("#download i").downloadify( {
	filename: function(){ return $('#download input').val();},
	data: function(){ return editor.getValue();},

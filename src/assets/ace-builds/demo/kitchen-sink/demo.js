/// BareSpecifier=ace-builds/demo/kitchen-sink/demo
...
define("ace/test/asyncjs/assert", ["require", "exports", "module", "ace/lib/oop"], function (require, exports, module) {
    ...
    toJSON = function (obj) {
        if (typeof JSON !== "undefined") return JSON.stringify(obj);else return obj.toString();
    };
...
define("ace/test/asyncjs/async", ["require", "exports", "module"], function (require, exports, module) {
    ...
    exports.Generator = function (source) {
        if (typeof source == "function") this.source = {
            next: source
                ...
                source.next(function handler(err, value) {
                    if (err) callback(err);else {
                        filter(value, function (err, takeIt) {
                            if (err) callback(err);else if (takeIt) callback(null, value);else source.next(handler);
                        });
                    }
                });
            };
        ...
        this.slice = function (begin, end) {
            ...
            this.next = function (callback) {
                source.next(function handler(err, value) {
                    count++;
                    if (err) callback(err);else if (count >= begin && count < end) callback(null, value);else if (count >= end) callback(STOP);else source.next(handler);
                });
            };
            return new this.constructor(this);
        };
                ...
                function reduceAll() {
                    source.next(function handler(err, currentValue) {
                        if (err) {
                            ...
                            if (err == STOP) return callback(null, previousValue);else return err;
                        }
                ...
                source.next(function handler(err, value) {
                    ...
                    condition(value, function (err, result) {
                        if (err) {
                            ...
                            if (err == STOP) callback(null, false);else callback(err);
                        } else if (result) {
                            done = true;
                            callback(null, true);
                ...
                source.next(function handler(err, value) {
                    ...
                    condition(value, function (err, result) {
                        if (err) {
                            ...
                            if (err == STOP) callback(null, true);else callback(err);
                        } else if (!result) {
                            done = true;
                            callback(null, false);
                        } else source.next(handler);
                ...
                source.next(function handler(err, value) {
                    if (err) {
                        if (err == STOP) {
                            ...
                            if (!source) return callback(STOP);else return source.next(handler);
        ...
        this.zip = function (generator) {
            ...
            return new this.constructor(function (callback) {
                exports.list(generators).map(function (gen, next) {
                    gen.next(next);
                }).toArray(callback);
            });
        };
        ...
        this.$arrayOp = function (arrayMethod, args) {
            ...
            this.next = function (callback) {
                ...
                self.toArray(function (err, arr) {
                    if (err) callback(err, "");else {
                        if (args) callback(null, arrayMethod.apply(arr, args));else callback(null, arrayMethod.call(arr));
                    }
                });
            };
            return new this.constructor(this);
        ...
        this.end = function (breakOnError, callback) {
            ...
            source.next(function handler(err, value) {
                if (err) {
                    if (err == STOP) callback && callback(lastError, last);else if (!breakOnError) {
                        lastError = err;
                        source.next(handler);
        ...
        this.toArray = function (breakOnError, callback) {
            ...
            source.next(function handler(err, value) {
                if (err) {
                    if (err == STOP) {
                        if (breakOnError) return callback(null, values);else {
                            errors.length = values.length;
                            return callback(errors, values);
                        }
                    } else {
                        if (breakOnError) return callback(err);else errors[values.length] = err;
                    }
                }
    ...
    var makeAsync = exports.makeAsync = function (args, fn, context) {
        if (fn.length > args) return fn;else {
            return function () {
                var value;
    ...
    exports.list = function (arr, construct) {
        ...
        return new construct(function (callback) {
            if (i < len) callback(null, arr[i++]);else callback(STOP);
        });
    };
    ...
    exports.range = function (start, stop, step, construct) {
        ...
        return new construct(function (callback) {
            if (step > 0 && value >= stop || step < 0 && value <= stop) callback(STOP);else {
                var current = value;
                value += step;
    ...
    exports.concat = function (first, varargs) {
        if (arguments.length > 1) return first.concat.apply(first, Array.prototype.slice.call(arguments, 1));else return first;
    };

    exports.zip = function (first, varargs) {
        if (arguments.length > 1) return first.zip.apply(first, Array.prototype.slice.call(arguments, 1));else return first.map(function (item, next) {
            next(null, [item]);
        });
    };
...
define("kitchen-sink/dev_util", ["require", "exports", "module", "ace/lib/dom", "ace/lib/event", "ace/range", "ace/edit_session", "ace/undomanager", "ace/lib/oop", "ace/lib/dom", "ace/range", "ace/editor", "ace/test/asyncjs/assert", "ace/test/asyncjs/async", "ace/undomanager", "ace/edit_session", "ace/test/mockrenderer", "ace/lib/event_emitter"], function (require, exports, module) {
    ...
    def(window, "editor", function () {
        warn();return window.env.editor == logEditor ? editor : window.env.editor;
    });
    ...
    exports.getUI = function (container) {
        return ["div", {}, " Test ", ["button", { onclick: exports.openLogView }, "O"], ["button", { onclick: exports.record }, "Record"], ["button", { onclick: exports.stop }, "Stop"], ["button", { onclick: exports.play }, "Play"], ["button", { onclick: exports.closeLogView }, "X"]];
    };
    ...
    exports.textInputDebugger = {
        ...
        showConsole: function () {
            ...
            if (!this.session) this.session = new EditSession("");
...
define("kitchen-sink/util", ["require", "exports", "module", "ace/lib/dom", "ace/lib/event", "ace/edit_session", "ace/undomanager", "ace/virtual_renderer", "ace/editor", "ace/multi_select"], function (require, exports, module) {
    ...
    exports.bindCheckbox = function (id, callback, noInit) {
        ...
        if (urlOptions[id]) el.checked = urlOptions[id] == "1";else if (localStorage && localStorage.getItem(id)) el.checked = localStorage.getItem(id) == "1";
    ...
    exports.bindDropdown = function (id, callback, noInit) {
        ...
        if (urlOptions[id]) el.value = urlOptions[id];else if (localStorage && localStorage.getItem(id)) el.value = localStorage.getItem(id);
...
define("ace/ext/options", ["require", "exports", "module", "ace/ext/menu_tools/overlay_page", "ace/lib/dom", "ace/lib/oop", "ace/lib/event_emitter", "ace/ext/modelist", "ace/ext/themelist"], function (require, exports, module) {
    ...
    var optionGroups = {
        Main: {
            ...
            "Keybinding": {
                ...
                items: [{ caption: "Ace", value: null }, { caption: "Vim", value: "ace/keyboard/vim" }, { caption: "Emacs", value: "ace/keyboard/emacs" }]
            },
            "Font Size": {
                path: "fontSize",
                type: "number",
            ...
            "Soft Wrap": {
                type: "buttonBar",
                path: "wrap",
                items: [{ caption: "Off", value: "off" }, { caption: "View", value: "free" }, { caption: "margin", value: "printMargin" }, { caption: "40", value: "40" }]
            },
            "Cursor Style": {
                path: "cursorStyle",
                items: [{ caption: "Ace", value: "ace" }, { caption: "Slim", value: "slim" }, { caption: "Smooth", value: "smooth" }, { caption: "Smooth And Slim", value: "smooth slim" }, { caption: "Wide", value: "wide" }]
            },
            "Folding": {
                path: "foldStyle",
                items: [{ caption: "Manual", value: "manual" }, { caption: "Mark begin", value: "markbegin" }, { caption: "Mark begin and end", value: "markbeginend" }]
            },
            "Soft Tabs": [{
                path: "useSoftTabs"
            }, {
        ...
        More: {
            ...
            "Merge Undo Deltas": {
                ...
                items: [{ caption: "Always", value: "always" }, { caption: "Never", value: "false" }, { caption: "Timed", value: "true" }]
            },
            "Elastic Tabstops": {
    ...
    (function () {
        ...
        this.render = function () {
            ...
            buildDom(["table", { id: "controls" }, this.renderOptionGroup(optionGroups.Main), ["tr", null, ["td", { colspan: 2 }, ["table", { id: "more-controls" }, this.renderOptionGroup(optionGroups.More)]]]], this.container);
        };
    ...
    (function () {
        ...
        this.getTokenizer = function () {
            ...
            SnippetManager.$tokenizer = new Tokenizer({
                start: [{ regex: /:/, onMatch: function (val, state, stack) {
                ...
                snippetVar: [{ regex: "\\|" + escape("\\|") + "*\\|", onMatch: function (val, state, stack) {
                    ...
                    onMatch: function (val, state, stack) {
                        ...
                        ts.flag = val[3];
                        return "";
                    }, next: "start" }, { regex: "`" + escape("`") + "*`", onMatch: function (val, state, stack) {
                        stack[0].code = val.splice(1, -1);
                        return "";
                    }, next: "start" }, { regex: "\\?", onMatch: function (val, state, stack) {
                        if (stack[0]) stack[0].expectIf = true;
                    }, next: "start" }, { regex: "([^:}\\\\]|\\\\.)*:?", token: "", next: "start" }],
                formatString: [{ regex: "/(" + escape("/") + "+)/", token: "regex" }, { regex: "", onMatch: function (val, state, stack) {
...
border:1px solid #baf; z-index:100";
    ...
    optionsPanelContainer.insertBefore(dom.buildDom(["div", { style: "text-align:right;margin-right: 60px" }, ["div", {}, ["button", { onclick: env.editSnippets }, "Edit Snippets"]], ["div", {}, ["button", { onclick: function () {
        ...
        } }, "Show Browser Info"]], devUtil.getUI()]), optionsPanelContainer.children[1]);
    ...
    function moveFocus() {
        var el = document.activeElement;
        if (el == env.editor.textInput.getElement()) env.editor.cmdLine.focus();else env.editor.focus();
    }
});
(function () {

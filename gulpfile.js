...
var styleTask = (stylesPath, srcs) =>
  gulp.src(srcs.map(src => path.join('app', stylesPath, src)))
...
var imageOptimizeTask = (src, dest) => gulp.src(src)
...
gulp.task('elements', () => styleTask('elements', ['**/*.css']));

// Ensure that we are not missing required files for the project
// "dot" files are specifically tricky due to them being hidden on
// some systems.
gulp.task('ensureFiles', cb =>
  ensureFiles(['.bowerrc'].map(p => path.join(__dirname, p)), cb));

// Optimize images
gulp.task('images', () => imageOptimizeTask('app/images/**/*', dist('images')));

// Copy all files at the root level (app)
gulp.task('copy', () => merge(gulp.src([
  'app/*',
  '!app/test',
  '!app/elements',
  ...
  '!**/.DS_Store',
], { dot: true }).pipe(gulp.dest(dist())), gulp.src([
  'app/bower_components/{' +
    'webcomponentsjs,platinum-sw,sw-toolbox,promise-polyfill' +
  '}/**/*',
]).pipe(gulp.dest(dist('bower_components')))).pipe($.size({ title: 'copy' })));
...
gulp.task('vulcanize', () => gulp.src('app/elements/elements.html')
  .pipe($.vulcanize({
    stripComments: true,
    inlineCss: true,
    inlineScripts: true,
...
gulp.task('deploy-gh-pages', () => gulp.src(dist('**/*'))

/// BareSpecifier=midi/generator/nodejs/packageWAVs
/*
	------------------------------------------------------
...
walker.on("file", function (root, stat, next) {
	...
	convertPackage(stat, "mp3", function () {
		// encode mp3
		convertPackage(stat, "ogg", function () {
			// encode ogg
			index++;
			return next();
		});
...
var convertPackage = function (stat, type, callback) {
	...
	exec(encoder[type] + " '" + src + "'", function (err) {
		// run encoder
...
MIDI.Soundfont["' + instrumentName + '"] = ';
	...
	zlib.gzip(buf, function (self, result) {
